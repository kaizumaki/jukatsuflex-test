...
export const RoomTypeMutation = extendType({
  type: 'Mutation',
  definition(t) {
    t.field('createRoomType', {
      type: RoomType.$name,
      args: {
        name: stringArg(),
      },
      async resolve(_, args, ctx) {
        try {
          return await ctx.prisma.roomType.create({
            data: args,
          })
        } catch (error) {
          console.log(error)
    ...
    t.field('updateRoomType', {
      ...
      async resolve(_, args, ctx) {
        ...
        delete data.id
        try {
          return await ctx.prisma.roomType.update({
            where: {
              id
            },
            data
    ...
    t.field('deleteRoomType', {
      ...
      async resolve(_, { id }, ctx) {
        try {
          ...
          return await ctx.prisma.roomType.delete({
            where: {
              id: toPrismaId(id),
            },
          })
